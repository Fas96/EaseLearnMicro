...
function doclist_getAllNodes(parsedArgs, filterParams, query, totalItemCount)
{
   ...
   if(projStatus!= null && projStatus.length > 0) {
	   query += ' +@eln\\:projStatus: "'+projStatus+'"';
   }

   if(projYear!= null && projYear.length > 0) {
 	  query += " +@eln\\:projYear:\"" + projYear + "\"";
   }
...
function doclist_main()
{
   ...
   for each (node in nodes)
   {
      ...
      if (item !== null &&  (filter!=="editingMe" && filter!=="editingOthers" || node.getIsLocked() || item.workingCopy.isWorkingCopy ) )
      {
         ...
         var assocManagers = node.assocs["eln:manager"];

         for each (manager in assocManagers) {
        	 item.projManager = {
        		userName : manager.properties["userName"],
        		firstName : manager.properties["firstName"]
        	 }
         }
