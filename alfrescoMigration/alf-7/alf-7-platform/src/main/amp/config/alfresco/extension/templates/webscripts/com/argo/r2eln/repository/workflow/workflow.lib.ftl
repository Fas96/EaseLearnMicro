...
<#escape x as jsonUtils.encodeJSONString(x)>
      {
         ...
         "workflowType": <#if task.contentId??>"${task.workflowType?string}"<#else>null</#if>,
         "outcome": <#if task.outcome??>"${task.outcome}"<#else>null</#if>,
  		 "contentId": <#if task.contentId??>"${task.contentId?string}"<#else>null</#if>,
         "owner":
         <#if task.owner??>
         {
         ...
         <#if task.reviewer??>
         {
            "userName": "${task.reviewer.userName}"<#if task.reviewer.firstName??>,
            "firstName": "${task.reviewer.firstName}"</#if><#if task.reviewer.lastName??>,
            "lastName": "${task.reviewer.lastName}"</#if>
         },
         <#else>
         null,
         </#if>
...
<#escape x as jsonUtils.encodeJSONString(x)>
{
   ...
   "package": <#if workflowInstance.package??>"${workflowInstance.package}"<#else>null</#if>,<#if workflowInstance.workflowType??>
   "workflowType": "${workflowInstance.workflowType?string}",</#if><#if workflowInstance.contentId??>
   "contentId": "${workflowInstance.contentId?string}",</#if><#if workflowInstance.currentTaskName??>
   "currentTaskName": "${workflowInstance.currentTaskName?string}",</#if>
   "initiator": 
   ...
   null,
	 ...
	 "reviewer":
	 <#if workflowInstance.reviewer??>
	 {
	    "userName": "${workflowInstance.reviewer.userName}"<#if workflowInstance.reviewer.firstName??>,
	    "firstName": "${workflowInstance.reviewer.firstName}"</#if><#if workflowInstance.reviewer.lastName??>,
	    "lastName": "${workflowInstance.reviewer.lastName}"</#if>
	 },
	 <#else>
	 null,
	 </#if>
