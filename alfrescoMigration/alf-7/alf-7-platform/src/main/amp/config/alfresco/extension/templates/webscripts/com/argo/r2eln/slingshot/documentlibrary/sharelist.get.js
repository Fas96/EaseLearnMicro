...
function doclist_getAllNodes(parsedArgs, filterParams, query, personNode, totalItemCount)
{
   var filter = args.filter,
       projStatus = args.projStatus,
       projYear = args.projYear,
      totalRecords = 0,
...
function doclist_main()
{
   ...
   var allNodesResult = doclist_getAllNodes(parsedArgs, filterParams, query, personNode, totalItemCount);
   ...
   var userAuthorityName = "eln:"+args.user+"__authorityScheme";
   ...
   for each (node in nodes)
   {
      ...
      var authorities = node.childAssocs["eln:authMembers"];

      // Get shared information
      for each (authority in authorities) {
    	  if(authority.properties["eln:authPerson"] && authority.properties["eln:authPerson"].name == personNode.name) {
