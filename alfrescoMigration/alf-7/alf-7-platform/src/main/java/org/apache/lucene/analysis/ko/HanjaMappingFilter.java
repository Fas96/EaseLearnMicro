    ...
    public final Token next(final Token reusableToken) throws IOException {
        ...
        while(outQueue.isEmpty()) {
            ...
            if(nextToken.getPositionIncrement()==0 ||
            		!hasHanja(nextToken.term())) return nextToken;

            int startOffset = nextToken.startOffset();
            
            List<StringBuffer> terms = splitByHanja(nextToken.term());
            int offset = 0;
            for(StringBuffer sb : terms) {
                if(sb.length()==0) continue;
                if(isHanja(sb.charAt(0)))
                    try {
                        mapHanjaToHangul(sb, startOffset);
...
//    }
    ...
    private void mapHanjaToHangul(StringBuffer term, int start) throws MorphException {

        outQueue.add(new KoreanToken(term.toString(), start, start+term.length()));
        ...
        for(int i=0;i<noCandidate;i++) {
        	KoreanToken tk = new KoreanToken(candiList.get(i).toString(),start, start+candiList.get(i).toString().length());
            ...
            for(int i=0;i<maxDecompounds;i++) {
            	...
            	for(int j=0;j<noCandidate;j++){
            		...
            		if(!done && pos[j]<=term.length()) {
            			KoreanToken tk = new KoreanToken(term.substring(offset[j],pos[j]),start + offset[j], start + offset[j]+term.length());
            			tk.setPositionIncrement(posInc);
            			outQueue.add(tk);
            		}
            		
            		if(dupcheck.get(entry.getWord())==null) {
            			KoreanToken tk = new KoreanToken(entry.getWord(),start+ offset[j], start+ offset[j]+entry.getWord().length());
            			tk.setPositionIncrement(0);
            			outQueue.add(tk);
