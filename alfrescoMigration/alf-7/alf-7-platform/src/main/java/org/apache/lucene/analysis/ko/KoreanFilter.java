...
public final class KoreanFilter extends TokenFilter {
  ...
  public static Log log = LogFactory.getLog(KoreanFilter.class);
  ...
  public final Token next(final Token reusableToken) throws IOException {
    assert reusableToken != null;
      if (!morphQueue.isEmpty()) {
        Token token = morphQueue.removeFirst();
        log.info(token.startOffset()+":"+token.endOffset()+":"+token.term()+"/"+token.getPositionIncrement());
        return token;
    }
    
    while(morphQueue.isEmpty()) {
    ...
    if(morphQueue.isEmpty()) return null;
 ...
 * @throws MorphException 
   */
  private void analysisKorean(String input, int start) throws MorphException {
...
//	input = trimHangul(input);
    ...
    if(hasOrigin) {
    	addTokenToMap(0,input,new KoreanToken(input,start, start+input.length()),map, false);
    }

    extractKeyword(outputs,start, map, 0); 
  ...
  private String removeSymbol(String input) {
	...
	private void extractKeyword(List<AnalysisOutput> outputs, int startoffset,
		...
		for (AnalysisOutput output : outputs) {
			...
			if (queryMode
				...
				KoreanToken tk = new KoreanToken(source,
						startoffset, startoffset+source.length());
				tk.setPositionIncrement(inc);
				
				addTokenToMap(position, source, tk, map, output.getScore()!=AnalysisOutput.SCORE_CORRECT);
				// map.put(position+":"+output.getStem(), new
				// KoreanToken(output.getStem(),startoffset,0));
			} else {
				String stem = removeSymbol(output.getStem());
				
				KoreanToken tk = new KoreanToken(stem,
						startoffset, startoffset+stem.length());
				tk.setPositionIncrement(inc);
				
				addTokenToMap(position, stem, tk, map, output.getScore()!=AnalysisOutput.SCORE_CORRECT);
			}
		...
		if (maxDecompounds > 1) {
			for (int i = 0; i < maxDecompounds; i++) {
				...
				for (AnalysisOutput output : outputs) {
					...
					KoreanToken kt = new KoreanToken(cEntry.getWord(), cStartoffset, cStartoffset+cEntry.getWord().length());
					kt.setPositionIncrement(inc);
					
					addTokenToMap(cPosition, cEntry.getWord(), kt, map);
	...
	private void addTokenToMap(int position, String text, KoreanToken kt, Map<String, KoreanToken> map) {
		addTokenToMap(position, text, kt, map, true);
	}
	
	private void addTokenToMap(int position, String text, KoreanToken kt, Map<String, KoreanToken> map, boolean removeOneChar) {
		if(removeOneChar && text.length()<2) return;
		
		map.put(position + ":" + text, kt);
	}
	...
	private int addBiagramToMap(String input, int startoffset,
		...
		while (offset < strlen - 1) {
			...
			if (isAlphaNumChar(input.charAt(offset))) {
				...
				KoreanToken tk =  new KoreanToken(text,
						startoffset + offset, startoffset + offset+text.length());
				tk.setPositionIncrement(inc);
				
				addTokenToMap(position, text, tk, map);
				offset += text.length();
			} else {
